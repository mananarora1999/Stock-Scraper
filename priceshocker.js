...
function priceshockerdataExtracter(html)
{
    
    let searchTool = cheerio.load(html);

    let folderName = "PRICE SHOCKERS";
    ...
    for (let i = 0; i < stockdetailsArr.length ; i++) 
    {
        ...
        let currprice=searchTool(stockdetailsNewArr[3]).text().trim();
        let prevprice=searchTool(stockdetailsNewArr[4]).text().trim();
        let perchange=searchTool(stockdetailsNewArr[5]).text().trim();

        // console.log((i+1)+"--->"+name+"  "+group+"  "+sector+"  "+currprice+"  "+prevprice+"  "+perchange);
        processStock(folderName, statType, name, group, sector, currprice, prevprice, perchange);
    } 
...
function processStock(folderName, statType, Name, Group, Sector, CurrPrice, PrevPrice, PerChange)
    {
        ...
        let stockobj = 
        {
            Name, Group, Sector, CurrPrice, PrevPrice, PerChange
        }
...
{
    dataExtracter:priceshockerdataExtracter
}