...
function onlySellerdataExtracter(html)
{
    
    let searchTool = cheerio.load(html);

    let folderName = "ONLY SELLERS";
    ...
    for (let i = 0; i < stockdetailsArr.length ; i++) 
    {
        ...
        let offerqty =searchTool(stockdetailsNewArr[2]).text().trim();
        ...
        // console.log((i+1)+"--->"+name+"  "+sector+"  "+offerqty+"  "+lastprice+"  "+diff+"  "+perchange);
        processStock(folderName, statType, name, sector, offerqty, lastprice, diff, perchange);
    }  
    
    console.log(statType + " scrapped!! PLEASE CHECK FOLDER");
}

function processStock(folderName, statType, Name, Sector, OfferQty, LastPrice, Diff, PerChange)
    {
        ...
        let stockobj = 
        {
            Name, Sector, OfferQty, LastPrice, Diff, PerChange
        }
...
{
    dataExtracter:onlySellerdataExtracter
}