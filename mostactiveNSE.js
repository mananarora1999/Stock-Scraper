...
function mostActiveNSEdataExtracter(html)
{
    
    let searchTool = cheerio.load(html);

    let folderName = "MOST ACTIVE";
    ...
    for (let i = 0; i < stockdetailsArr.length ; i++) 
    {
        ...
        let value=searchTool(stockdetailsNewArr[5]).text().trim();

        // console.log((i+1)+"--->"+name+"  "+high+"  "+low+"  "+lastprice+"  "+perchange+"  "+value);
        processStock(folderName, statType, name, high, low, lastprice, perchange, value);
    } 
...
function processStock(folderName, statType, Name, High, Low, LastPrice, PerChange, Value)
    {
        ...
        let stockobj = 
        {
            Name, High, Low, LastPrice, PerChange, Value
        }
...
{
    dataExtracter:mostActiveNSEdataExtracter
}