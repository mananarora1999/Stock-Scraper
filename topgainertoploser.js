...
function topGainertopLoserdataExtracter(html)
{
    
    let searchTool = cheerio.load(html);

    let folderName = "TOP GAINERS TOP LOSERS";
    let statType = searchTool(".clearfix.wbg.MB10 > .eenlft").text().trim();

    let stockdetailsArr = searchTool("div.bsr_table.hist_tbl_hm>table>tbody>tr");
    ...
    for (let i = 0; i < stockdetailsArr.length ; i++) 
    {
        ...
        let high=searchTool(stockdetailsNewArr[1]).text().trim();
        let low =searchTool(stockdetailsNewArr[2]).text().trim();
        let lastprice=searchTool(stockdetailsNewArr[3]).text().trim();
        let prevclose=searchTool(stockdetailsNewArr[4]).text().trim();
        let change=searchTool(stockdetailsNewArr[5]).text().trim();
        let pergainorloss=searchTool(stockdetailsNewArr[6]).text().trim();

        // console.log((i+1)+"--->"+name+"  "+high+"  "+low+"  "+lastprice+"  "+prevclose+"  "+change+"  "+pergainorloss);
        processStock(folderName, statType, name, high, low, lastprice, prevclose, change, pergainorloss);
    }

    console.log(statType + " scrapped!! PLEASE CHECK FOLDER");    
}

    function processStock(folderName, statType, Name, High, Low, LastPrice, PrevClose, Change, PerGainorLoss)
    {
        ...
        let stockobj = 
        {
            Name, High, Low, LastPrice, PrevClose, Change, PerGainorLoss
        }
...
{
    dataExtracter:topGainertopLoserdataExtracter
}