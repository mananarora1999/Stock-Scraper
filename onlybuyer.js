...
function onlyBuyerdataExtracter(html)
{
    
    let searchTool = cheerio.load(html);
 
    let folderName = "ONLY BUYERS";
    let statType = searchTool(".clearfix.wbg.MB10 > .eenlft").text().trim();
    
    let stockdetailsArr = searchTool("div.bsr_table.bsr_table930.MT20.PR.hist_tbl>table>tbody>tr");
    ...
    for (let i = 0; i < stockdetailsArr.length ; i++) 
    {
        ...
        let sector=searchTool(stockdetailsNewArr[1]).find("a").text().trim();
        let bidqty =searchTool(stockdetailsNewArr[2]).text().trim();
        let lastprice=searchTool(stockdetailsNewArr[3]).text().trim();
        let diff=searchTool(stockdetailsNewArr[4]).text().trim();
        let perchange=searchTool(stockdetailsNewArr[5]).text().trim();
        
        // console.log((i+1)+"--->"+name+"  "+sector+"  "+bidqty+"  "+lastprice+"  "+diff+"  "+perchange);
        processStock(folderName, statType, name, sector, bidqty, lastprice, diff, perchange);
    } 

    console.log(statType + " scrapped!! PLEASE CHECK FOLDER");
}

function processStock(folderName, statType, Name, Sector, BidQty, LastPrice, Diff, PerChange)
    {
        ...
        let stockobj = 
        {
            Name, Sector, BidQty, LastPrice, Diff, PerChange
        }
...
module.exports= // object that will be exported
{
    ...
    dataExtracter:onlyBuyerdataExtracter
}